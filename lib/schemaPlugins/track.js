...
module.exports = function track() {
	...
	list.schema.pre('save', function (next) {
        ...
        if ((this._req_user && !this._req_user.isAdmin)
            //the user is not an Admin
            && ((!this.createdBy)
                //if no createdBy field exists, 
                //prevent updation since no nonAdmin items lack the createdBy field
                || (this.createdBy.toString() !== this._req_user._id.toString())))
        //if createdBy field exists and it does not match the current user
        //prevent updation
            {
                console.log("Item created by/assigned to : " + this.createdBy + "   Current user : " + this._req_user._id + " isAdmin ? " + this._req_user.isAdmin);
